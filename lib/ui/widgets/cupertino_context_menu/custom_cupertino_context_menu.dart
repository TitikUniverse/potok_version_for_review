...
// The scale of the child at the time that the CustomCupertinoContextMenu opens.
...
/// A function that produces the preview when the CustomCupertinoContextMenu is open.
///
...
// on the screen of [CustomCupertinoContextMenu.child] before the
// [CustomCupertinoContextMenu] opens.
...
///
/// When open, the [CustomCupertinoContextMenu] shows the child, or the widget returned
...
/// unconstrained.
///
/// When closed, the CustomCupertinoContextMenu simply displays the child as if the
/// CustomCupertinoContextMenu were not there. Sizing and positioning is unaffected.
...
///
/// {@tool dartpad}
/// This sample shows a very simple CustomCupertinoContextMenu for an empty red
...
class CustomCupertinoContextMenu extends StatefulWidget {
  ...
  /// The widget that can be "opened" with the [CustomCupertinoContextMenu].
  ///
  /// When the [CustomCupertinoContextMenu] is long-pressed, the menu will open and
  ...
  /// doesn't size itself.
  ///
  /// When the [CustomCupertinoContextMenu] is "closed", this widget acts like a
  ...
  /// [CustomCupertinoContextMenu] is open.
  ///
  ...
  /// the CustomCupertinoContextMenu opens, and from 1 to 0 when it closes, and it can
  ...
  /// CustomCupertinoContextMenu is closed.
  ///
  /// {@tool snippet}
  ///
  ...
  /// rounded corners. The larger size of the open CustomCupertinoContextMenu allows
  ...
  /// into view as the CustomCupertinoContextMenu is opened. The preview is swapped in
  ...
  ///   // aspect ratio when the CustomCupertinoContextMenu is animating its preview
  ///   // widget open and closed.
  ///   previewBuilder: (BuildContext context, Animation<double> animation, Widget child) {
  ///     return FittedBox(
  ///       fit: BoxFit.cover,
  ///       // This ClipRRect rounds the corners of the image when the
  ///       // CustomCupertinoContextMenu is open, even though it's not rounded when
  ...
  @override
  State<CustomCupertinoContextMenu> createState() => _CupertinoContextMenuState();
}

class _CupertinoContextMenuState extends State<CustomCupertinoContextMenu> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _openController = AnimationController(
      duration: const Duration(milliseconds: 150),// kLongPressTimeout,
      vsync: this,
    );
  ...
  // align the content of the open CustomCupertinoContextMenu. For example, if the
  ...
  // Push the new route and open the CustomCupertinoContextMenu overlay.
  ...
  // the CustomCupertinoContextMenu just behaves as a Container.
...
// A floating copy of the CustomCupertinoContextMenu's child.
//
// When the child is pressed, but before the CustomCupertinoContextMenu opens, it does
...
class _DecoyChildState extends State<_DecoyChild> with TickerProviderStateMixin {
  ...
  static const Color _lightModeMaskColor = Color(0xFFFFFFFF); // 0xFF888888
  ...
  void initState() {
    ...
    _rect = TweenSequence<Rect?>(<TweenSequenceItem<Rect?>>[
      ...
      TweenSequenceItem<Rect?>(
        ...
        ).chain(CurveTween(curve: Curves.easeOutCubic)), // easeOutCubic
        weight: 1.0,
      ),
...
// The open CustomCupertinoContextMenu modal.
    ...
    setState(() {
      ...
      _moveAnimation = Tween<Offset>(
        ...
        CurvedAnimation(
          ...
          curve: Curves.elasticIn, // elasticIn
        ),
      );
...
// The menu that displays when CustomCupertinoContextMenu is open. It consists of a
